...
if not main_activity_dir:
    ...
    content = (
        ...
        "                    // Intercept /mobile-signin?auth_key=UUID navigations from the\n"
        "                    // WebView. Open Chrome Custom Tab for the FULL OAuth flow so that\n"
        "                    // NextAuth state/CSRF cookies are set and verified in Chrome's\n"
        "                    // cookie jar (not the WebView's). Load /mobile-wait in the WebView\n"
        "                    // so the polling script can detect when Chrome finishes.\n"
        "                    if (\"emergenthealth.vercel.app\".equals(host)\n"
        "                            && \"/mobile-signin\".equals(uri.getPath())) {\n"
        ...
        "                    if (host != null && (host.equals(\"accounts.google.com\")\n"
