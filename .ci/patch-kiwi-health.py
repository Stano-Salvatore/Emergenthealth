...
if not main_activity_dir:
    ...
    content = (
        ...
        "    // Where a widget or launcher shortcut asked us to land.\n"
        "    private volatile String mPendingDest = null;\n\n"
        ...
        "        }\n"
        "    }\n\n"
        "    // Printing. window.print() exists on the WebView's window object and\n"
        "    // does absolutely nothing — there is no print stack behind it, the way\n"
        "    // SpeechRecognition is present but inert. The health report's\n"
        "    // \"Print / Save as PDF\" button therefore did nothing at all on the\n"
        "    // phone. PrintManager drives the same WebView through Android's own\n"
        "    // print dialog, which offers Save as PDF alongside real printers.\n"
        "    private class EhPrint {\n"
        "        @JavascriptInterface\n"
        "        public boolean isAvailable() { return true; }\n\n"
        "        @JavascriptInterface\n"
        "        public void print(final String documentName) {\n"
        "            runOnUiThread(new Runnable() {\n"
        "                @Override public void run() {\n"
        "                    try {\n"
        "                        WebView wv = bridge.getWebView();\n"
        "                        if (wv == null) return;\n"
        "                        String name = (documentName == null || documentName.isEmpty())\n"
        "                            ? \"Emergenthealth\" : documentName;\n"
        ...
        "                            wv.createPrintDocumentAdapter(name);\n"
        "                        pm.print(name, adapter,\n"
        ...
        "        }\n"
        "    }\n\n"
        "    // Home-screen widgets. They read their credentials out of\n"
        "    // SharedPreferences and otherwise only redraw on their own slow tick,\n"
        "    // so a freshly linked widget could sit dead for half an hour. Providers\n"
        "    // are found through the package manager rather than named directly:\n"
        "    // MainActivity is generated here, the widgets are copied in by a\n"
        "    // separate script, and a compile-time reference would couple the two.\n"
        "    private class EhWidgets {\n"
        "        @JavascriptInterface\n"
        "        public void refresh() { refreshAllWidgets(); }\n"
        "    }\n\n"
        ...
        "            if (mgr == null) return;\n"
        "            for (android.appwidget.AppWidgetProviderInfo info :\n"
        "                    mgr.getInstalledProvidersForPackage(getPackageName(), null)) {\n"
        "                int[] ids = mgr.getAppWidgetIds(info.provider);\n"
        "                if (ids == null || ids.length == 0) continue;\n"
        "                Intent update = new Intent(\n"
        "                    android.appwidget.AppWidgetManager.ACTION_APPWIDGET_UPDATE);\n"
        "                update.setComponent(info.provider);\n"
        "                update.putExtra(\n"
        "                    android.appwidget.AppWidgetManager.EXTRA_APPWIDGET_IDS, ids);\n"
        "                sendBroadcast(update);\n"
        "            }\n"
        "        } catch (Exception ignored) {}\n"
        "    }\n\n"
        "    // Downloads. A WebView with no DownloadListener does nothing at all\n"
        "    // when a link with a download attribute is tapped, or when a response\n"
        "    // comes back as Content-Disposition: attachment — no file, no error, no\n"
        "    // console warning. Capacitor does not install one, so every export in\n"
        "    // Settings, including the full JSON backup, was inert on the phone.\n"
        "    // DownloadManager does the fetching; the session cookie has to be\n"
        "    // carried across by hand or the request comes back as the sign-in page.\n"
        "    private void setupDownloadListener(WebView wv) {\n"
        ...
        "                    // setMimeType overrides what the server said, so hand it a\n"
        "                    // bare type — a charset parameter riding along has no\n"
        "                    // business in DownloadManager's content type.\n"
        "                    if (mimeType != null) {\n"
        "                        int semi = mimeType.indexOf(';');\n"
        "                        req.setMimeType(semi > 0 ? mimeType.substring(0, semi).trim() : mimeType.trim());\n"
        "                    }\n"
        "                    req.setTitle(name);\n"
        "                    req.setDescription(\"Emergenthealth export\");\n"
        "                    req.addRequestHeader(\"User-Agent\", userAgent);\n"
        "                    String cookie = android.webkit.CookieManager.getInstance().getCookie(url);\n"
        "                    if (cookie != null) req.addRequestHeader(\"Cookie\", cookie);\n"
        "                    req.setNotificationVisibility(android.app.DownloadManager.Request\n"
        "                        .VISIBILITY_VISIBLE_NOTIFY_COMPLETED);\n"
        "                    try {\n"
        "                        req.setDestinationInExternalPublicDir(\n"
        "                            android.os.Environment.DIRECTORY_DOWNLOADS, name);\n"
        "                    } catch (Exception publicDirDenied) {\n"
        "                        // Pre-scoped-storage devices can refuse the public\n"
        "                        // Downloads folder without WRITE_EXTERNAL_STORAGE. A file\n"
        "                        // in the app's own folder beats no file at all, and beats\n"
        "                        // asking Play for a permission modern Android ignores.\n"
        ...
        "                        \"Downloading \" + name, android.widget.Toast.LENGTH_SHORT).show();\n"
        "                } catch (Exception e) {\n"
        "                    android.widget.Toast.makeText(MainActivity.this,\n"
        ...
        "            wv.addJavascriptInterface(new EhPrint(), \"EhPrint\");\n"
        "            wv.addJavascriptInterface(new EhWidgets(), \"EhWidgets\");\n"
        ...
        "                    // Capture the auth key natively. It used to be set only by\n"
        "                    // EhAuthBridge.openSignIn, so whenever the JavaScript interface\n"
        "                    // was unavailable — it is not visible to a page that loaded\n"
        "                    // before addJavascriptInterface ran — the web fallback navigated\n"
        "                    // here and the native side never learned the key, leaving\n"
        "                    // redeemPendingAuthKey with nothing to redeem on resume. Sign-in\n"
        "                    // then failed silently. Reading it off the URL makes the handoff\n"
        "                    // work whether or not the bridge is present.\n"
        "                    if (path != null && path.startsWith(\"/mobile-signin\")) {\n"
        "                        String k = uri.getQueryParameter(\"auth_key\");\n"
        "                        if (k != null && !k.isEmpty()) mPendingAuthKey = k;\n"
        "                    }\n\n"
        ...
        "        });\n"
        "    }\n\n"
        "    // A widget or shortcut can name the screen it belongs to. Only a path is\n"
        "    // ever accepted, never a URL: this WebView holds a signed-in session, so\n"
        "    // letting another app hand it an arbitrary destination would hand over the\n"
        "    // session with it. A leading // is rejected too — that is protocol-relative\n"
        "    // and would leave the origin just as surely as https:// would.\n"
        "    private static String safeDest(Intent intent) {\n"
        "        if (intent == null) return null;\n"
        "        String dest = intent.getStringExtra(\"eh_dest\");\n"
        "        if (dest == null || dest.length() < 2) return null;\n"
        "        if (dest.charAt(0) != '/' || dest.charAt(1) == '/') return null;\n"
        "        if (dest.indexOf('\\\\') >= 0) return null;\n"
        "        return dest;\n"
        "    }\n\n"
        "    // Posted rather than called straight out of onCreate: Capacitor starts its\n"
        "    // own load of server.url inside super.onCreate(), and a loadUrl issued in\n"
        "    // the same pass would be the one that loses. Posting puts ours after it.\n"
        "    private void navigatePendingDest() {\n"
        "        final String dest = mPendingDest;\n"
        "        if (dest == null) return;\n"
        "        mPendingDest = null;\n"
        "        try {\n"
        ...
        "            wv.post(new Runnable() {\n"
        "                @Override public void run() {\n"
        "                    wv.loadUrl(\"https://emergenthealth.vercel.app\" + dest);\n"
        "                }\n"
        "            });\n"
        "        } catch (Exception ignored) {}\n"
        "    }\n\n"
        ...
        "        // Registered before super.onCreate(): Capacitor builds its\n"
        "        // bridge there, and a plugin registered afterwards is simply\n"
        "        // not in it.\n"
        "        registerPlugin(EmergyBubblePlugin.class);\n"
        "        super.onCreate(savedInstanceState);\n"
        "        setupBridgeHooks();\n"
        "        configureWebViewViewport();\n"
        "        mPendingDest = safeDest(getIntent());\n"
        "        navigatePendingDest();\n"
        "    }\n\n"
