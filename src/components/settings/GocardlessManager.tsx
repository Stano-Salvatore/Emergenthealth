...
export function GocardlessManager() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const d = await fetch("/api/gocardless/connect").then(r => r.json())
        ...
        if (!d.accountId) {
          setState("picking")
          const ad = await fetch("/api/gocardless/accounts").then(r => r.json())
  ...
  function connect() { window.location.href = "/api/gocardless/auth" }

  async function disconnect() {
    await fetch("/api/gocardless/connect", { method: "DELETE" })
    setAccountName(null); setCurrency(null); setAccounts([])
    setState("disconnected"); setSyncMsg(null); setError(null)
  }

  async function selectAccount(a: Account) {
    await fetch("/api/gocardless/connect", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ accountId: a.id, accountName: a.name, currency: a.currency }),
    })
    setAccountName(a.name); setCurrency(a.currency)
    setState("connected"); setError(null)
  }

  async function sync() {
    setState("syncing"); setSyncMsg(null); setError(null)
    const res = await fetch("/api/sync/gocardless", { method: "POST" })
      ...
      <CardContent className="pt-4 pb-4 space-y-3">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {state === "connected"
                ...
                : state === "picking"
                  ...
                  : "Connect Revolut or another bank via GoCardless Bank Account Data."}
            </p>
          </div>
          ...
          accounts.length === 0 ? (
            <div className="space-y-2">
              <p className="text-xs text-amber-400 bg-amber-500/10 rounded-md px-3 py-2">
                No accounts found — try reconnecting.
              </p>
        ...
        {state === "disconnected" && (
          hasConfig ? (
            ...
            <p className="text-xs text-amber-400 bg-amber-500/10 rounded-md px-3 py-2">
              Set <code className="font-mono">GOCARDLESS_SECRET_ID</code> and{" "}
              <code className="font-mono">GOCARDLESS_SECRET_KEY</code> in Vercel environment variables.
            </p>
          )
        )}
        ...
        {syncMsg && (
          <p className="text-xs text-green-400 bg-green-500/10 rounded-md px-3 py-2">{syncMsg}</p>
        )}
      </CardContent>
    </Card>
  )
