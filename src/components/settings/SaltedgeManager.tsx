...
export function SaltedgeManager() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const d = await fetch("/api/saltedge/connect").then(r => r.json())
        ...
        if (!d.accountId) {
          setState("picking")
          const ad = await fetch("/api/saltedge/accounts").then(r => r.json())
  ...
  function connect() { window.location.href = "/api/saltedge/auth" }

  async function disconnect() {
    await fetch("/api/saltedge/connect", { method: "DELETE" })
  ...
  async function selectAccount(a: Account) {
    await fetch("/api/saltedge/connect", {
      method: "PATCH",
  ...
  async function sync() {
    ...
    const res = await fetch("/api/sync/saltedge", { method: "POST" })
      ...
      <CardContent className="pt-4 pb-4 space-y-3">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {state === "connected"
                ...
                : state === "picking"
                  ...
                  : "Connect your bank (Revolut, etc.) via Salt Edge for automatic transaction sync."}
            </p>
          </div>
        ...
        {state === "disconnected" && (
          hasConfig ? (
            ...
            <p className="text-xs text-amber-400 bg-amber-500/10 rounded-md px-3 py-2">
              Set <code className="font-mono">SALTEDGE_APP_ID</code> and{" "}
              <code className="font-mono">SALTEDGE_SECRET</code> in Vercel environment variables.
            </p>
          )
        )}
