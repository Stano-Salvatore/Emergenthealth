// <emergy-avatar mood="okay" fit="icon" gesture="">  — the 3D mascot, mounted anywhere in the UI.
...
//
// Copied from the design handoff (design/handoff/emergy-avatar.js) with one
// change: three.js comes from the npm bundle instead of a CDN import map.
// Client-only — import it from an effect, never at module scope of an SSR'd file.

import * as THREE from "three"
import { buildEmergy, setMood, poseEmergy, makeStudio, FITS } from "./emergy-model.js"
...
function boot() {
  ...
  const ctx = { renderer, off, scene, camera, model, lastFit: null }
  ...
  function tick(now) {
    ...
    last = now
    const t = (now - t0) / 1000
    // render fit-by-fit so the renderer resizes a few times per frame, not per slot
...
class EmergyAvatarElement extends HTMLElement {
  ...
  _draw(ctx, t) {
    const { renderer, camera, off, scene, model } = ctx
...
if (typeof window !== "undefined" && !customElements.get("emergy-avatar")) {
  customElements.define("emergy-avatar", EmergyAvatarElement)
}

/** Make every mascot on the page wave (or sigh). */
export function emergyPlay(gesture, ms) { for (const el of instances) el.play(gesture, ms) }
