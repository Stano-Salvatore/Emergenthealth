...
export async function POST(req: NextRequest) {
  ...
  const rl = checkRateLimit(userId, "experiments_write", 30, 60 * 60 * 1000)
  if (!rl.allowed) return NextResponse.json({ error: "Too many changes this hour — try again later.", resetAt: rl.resetAt }, { status: 429 })
